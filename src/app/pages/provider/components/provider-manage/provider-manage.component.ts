...
export class ProviderManageComponent implements OnInit {
  ...
  constructor(
    ...
    private _documentTypeServices: DocumentTypeService
  ...
  listDocumentTypes():void{
    this._documentTypeServices.listDocumentTypes().subscribe((resp)=>{
  ...
  providerRegister():void{
    this._providerService.providerRegister(this.form.value).subscribe((resp)=>{
      if(resp.isSuccess){
        this._alert.success("Exelente",resp.message);
        this._dialogRef.close(true);
      }
      else{
        this._alert.wram("Error",resp.message);
      }
    })
  }
